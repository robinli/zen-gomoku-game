...
const GameInfo: React.FC<GameInfoProps> = ({ room, localPlayer, onReset, onGoHome, onRequestUndo, isConnected, isReconnecting, isWaitingUndo, isWaitingReset }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        onClick={onReset}
        disabled={!isConnected || isWaitingReset}
        className={`w-full py-3 border-2 border-slate-300 text-slate-700 rounded-xl text-sm font-semibold transition-all hover:bg-slate-50 hover:border-slate-400 flex items-center justify-center gap-2 ${!isConnected || isWaitingReset ? 'opacity-30 cursor-not-allowed' : ''}`}
      >
